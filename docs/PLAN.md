...
3. **Identity, version and boundary are three questions** with three confidences and three tiers. The badge shows the work tier; only the **verified** badge additionally requires the version to be verified, and the version status is always displayed alongside (rev 5.1).
...
### `observation` (key: `query_id ‖ mix_span ‖ raw_label_hash ‖ native_index ‖ transform`) — **rev 5.2** adds `transform` (null for scanners) because byte-identical sibling WAVs share one `cache_key`, so a single `query_id` fans out to every sibling, and a `pitch` sibling has exactly the same `mix_span_ms` as its `none` parent
...
### Window schedule
`hop ≤ window − L` gives coverage-completeness for contained events of length `L`. Default **12,000 ms window, 9,000 ms hop** for generation 0, end-anchored tail, short-input rule. Benchmark compares coverage-complete vs sparse schedules (6/8/12 s × 5/9/15 s × phases) with cost reported separately. **rev 5.2:** only the *active* measured `L_min` (currently v3, 3 s) gates coverage-completeness; superseded configs are reported, never used as gates. The Stage 4b benchmark's dense **12 s / 5 s / phase 0** schedule is adopted as the **rescan** policy (Stage 4c), not as the generation-0 default, because tier thresholds were calibrated against 9 s hops and denser hops would inflate `T` without new evidence (see *Provisional tiers*).
...
`T` = independent logical trials with a final match, **counted as `T_ind` (rev 5.2): the maximum set of pairwise non-overlapping `support_ms` intervals among those trials (greedy interval scheduling)** — so overlapping windows from a dense hop or rescan cannot inflate tiers; `S` = span of evidence support; `C` = competing candidate covering ≥ 50% of the same span. Proved bounds and `evidence_support_ms` are computed **only from the per-trial selected observations** (the "votes"), never from rejected hypothesis siblings, whose ids are kept in `rejected_evidence` for provenance. Work: `likely` if `T ≥ 4 ∧ S ≥ 40,000 ∧ ¬C ∧ has_global_alignment`; `possible` if `T ≥ 2 ∧ S ≥ 20,000 ∧ ¬C`; else `unclear`. Version: `unclear` unless recording-specific ids corroborated by ≥ 2 independent sources (then = work tier). Boundary: `possible` if `has_global_alignment`, else `unclear`. Never `verified` while provisional.
...
## Decisions log
**rev 5.2 (2026-09-04, after the Stage 4b review):** gen-0 schedule stays 12/9; the benchmark's 12/5/0 becomes the rescan policy; only the active measured `L_min` gates coverage-completeness; tiers count `T_ind` (non-overlapping supports); proved bounds come only from selected per-trial observations, with `rejected_evidence` kept separately; the observation natural key gains `transform` so sibling hypotheses can never collide on one id.
**rev 5.1 (2026-09-04, after the first real-set run):** badge = work tier capped at `likely` unless version verified, with `version_status` displayed — because `badge = min(work, version)` made every row UNCLEAR in the single-engine free profile (version can never be corroborated by one engine), which defeats the tool's purpose without weakening the "verified" guardrail.
Python/uv; ensemble with measured dependence; three tiers (r2); **one-sided proved bounds only; L_min/cues/extrapolation are prediction features** (r3–r4); **transform algebra: input span `12000/r`, output→original `k/r`, rational sample maps** (r4); generations + rescan-plan artefact (r4); durations partition, censored tails ≠ gaps (r4); one point per logical trial; ref-consistency-first continuation (r4); certification per profile×dimension×tier with cluster gates (r4); association by identity + evidence compatibility (r4); text ⇒ `same_work` only (r4); time-varying roles, `claim` (r4); single retry owner with injected client (r4); Job-Object launcher named (r4); committed-corpus policy (r4); corpus construction a named deliverable (r4); Panako minimum path before freeze if enabled (r4).
