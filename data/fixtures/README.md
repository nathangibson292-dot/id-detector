...
- **random fixture-local identities** (a fresh random token per author per fixture file — never a stable hash of the handle)
...
`scripts/audit_fixtures.py` runs in CI/tests and **fails** if any committed fixture contains a handle pattern (`@…`), a URL, a platform numeric ID, or text that matches a raw-dump line verbatim beyond the retained field.
